...
A one-file FastAPI notes app used as the Caseathon Builders warm-up. Product intent is in `docs/prd.md`, the increments in `docs/plan.md`, the architecture decisions in `docs/adr.md`. Read all three before changing anything.
...
- Keep the decisions in `docs/adr.md`. Changing one is allowed only after writing the new paragraph first.
